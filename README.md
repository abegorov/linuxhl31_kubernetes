...
С помощью **kubeadm** поднимается кластер **kubernetes** из 3-х **control-plane** и 3-x **worker** с **flannel**, **certmanager**, **gateway api**, **envoy gateway**, **longhorn**. В нём разворачивается **wordress** (на **php-fpm**) с **mariadb**. Для публикации **wordpress** используется **angie**, которые запускается в качестве **sidecar** контейнера к **wordpress** и проксирует запросы к нему через `/run/php-fpm.sock`.
